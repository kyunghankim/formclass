...
def new(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            article = form.save() #<-이걸로 밑에 주석처리 코드 대체가능
            # #2-2. 검증된 data 꺼내오기
...
def edit(request, pk):
    ...
    if request.method == "POST":
        # data 수정!
        # # 1. Database에서 data 가져오기, 이미있으니 안해도됨
        # article = Article.objects.get(pk=pk)
        ...
        # # 2. Form에 data 채워 넣기                    #<- initial:초기화 하겠다! 
        # form = ArticleForm(initial=article.__dict__) #<- __dict__:dictionary로 바꾸어넣어줌
    context = {
        'form': form,
    }
